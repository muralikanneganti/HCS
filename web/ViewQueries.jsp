...
<table id="myTable" class="table table-striped" style="background-color: transparent;">
    <thead>
        <tr>
            <th>Patient Name</th><th>Age</th><th>Smoking</th><th>Alcohlic</th><th>Query</th><th>Your Reply</th><th>Date</th><th>Action</th>
        </tr>
    </thead>
    <tbody>
        <%
            try {
                ...
                rs = st.executeQuery("select * from queries where did='" + did + "'");
                while (rs.next()) {
        ...
        %>
        <tr>
            <td><%=pname%></td><td><%=age%></td><td><%=smoking%></td><td><%=alcohlic%></td><td><%=symtomps%></td><td><%=rs.getString("doctor_reply")%></td><td><%=date%></td>
            <td><a href="SendReply.jsp?_tokes=<%=tokes%>&&tokes=<%=tokes%>&&pid=<%=pid%>&&pname=<%=pname%>&&age=<%=age%>&&tokes=<%=tokes%>&&date=<%=date%>">Reply</a></a></td>
        </tr>
        <%
                }
            } catch (Exception e) {
