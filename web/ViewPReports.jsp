...
<html>
    ...
    <body id="page-top" data-spy="scroll" data-target=".navbar-custom" style="background:url('images/bg1.jpg') no-repeat;">
        ...
        %>
    <center><h2>Reports of <%=pname%>&nbsp;&nbsp;[<%=pid%>] &nbsp;<a href="AskDoctor.jsp">Ask Query</a></h2></center>
    <table id="myTable" class="table table-striped" >
        <thead>
            <tr>
                <th>Sr. No.</th><th>Consultant Doctor Name</th><th>Department</th><th>Doctor Comment</th><th>Appointment Date</th>
            </tr>
        </thead>
        <tbody>
            <%
                ...
                try {
                    ...
                    rs = st.executeQuery("select a.dname,a.department,a.appoint_date_time,q.doctor_reply from appointment a inner join queries q on a.pid=q.pid where a.pid='" + pid + "'");
                    while (rs.next()) {
                        count++;
            %>
            <tr>
                <td><%=count%></td><td><%=rs.getString("dname")%></td><td><%=rs.getString("department")%></td><td><%=rs.getString("doctor_reply")%></td><td><%=rs.getString("appoint_date_time")%></td>
            </tr>
            <%
                    }
                } catch (Exception e) {
